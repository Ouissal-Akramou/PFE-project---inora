...
import DraftBanner from '@/components/DraftBanner';  // ← ADD
...
const heroStats  = [
  { num:'12', label:'Guests Max'       },
  { num:'3',  label:'Crafts Available' },
...
const cornerOrnamentsConfig = [
  { pos:'top-3 left-3',     rot:'rotate-0'   },
  { pos:'top-3 right-3',    rot:'rotate-90'  },
  { pos:'bottom-3 right-3', rot:'rotate-180' },
  { pos:'bottom-3 left-3',  rot:'-rotate-90' },
];
...
function LaceCorner({ pos, rot }) {
  return (
    <div className={`absolute ${pos} w-28 h-28`}>
...
export default function Home() {
  ...
  // ── UPDATED: go to /book with activity pre-selected ──
  ...
  return (
    <main className="text-[#3a3027] overflow-x-hidden relative bg-[#FBEAD6]">
      ...
      <div className="fixed inset-0 pointer-events-none z-40">
        ...
        {cornerOrnamentsConfig.map(({ pos, rot }, i) => (
          <LaceCorner key={i} pos={pos} rot={rot} />
        ))}
      ...
      <section className="relative w-full overflow-hidden bg-[#FBEAD6]" style={{ minHeight:'92vh', paddingTop:'80px' }}>
        ...
        <div className="absolute top-1/2 left-1/2 pointer-events-none"
          style={{ width:'700px', height:'700px', borderRadius:'50%', background:'radial-gradient(ellipse at center,rgba(200,125,135,0.28) 0%,rgba(200,125,135,0.07) 40%,transparent 68%)', animation:'auraPulse 5s ease-in-out infinite', filter:'blur(8px)', transform:'translate(-50%,-50%)' }}/>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {activities.map((a, i) => (
              <div key={i}
                ...
                <div className="absolute top-0 left-0 w-full h-[2px] scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left rounded-t-2xl"
                ...
                <div className="p-6">
                  ...
                  <div className="h-px mb-3 transition-all duration-500 group-hover:w-12"
                    style={{ background: a.accent, width: activitiesIn ? '1.75rem' : '0', transition:'width .6s ease .4s' }}/>
              ...
              <ul className="flex flex-col gap-2.5">
                {footerLinks.map(item => (
                  <li key={item.label} className="relative group">
                    <Link href={item.href}
                      className="font-['Cormorant_Garamond',serif] text-sm text-[#FBEAD6]/60 hover:text-[#C87D87] transition-colors duration-200 tracking-wide flex items-center gap-2 link-line">
