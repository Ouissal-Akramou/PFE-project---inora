...
export default function Login() {
  ...
  return (
    ...
    >
      ...
      />
      <div
        className="absolute -bottom-10 -right-10 w-48 sm:w-72 h-48 sm:h-72 rounded-full pointer-events-none"
      ...
      >
        ...
        {[
          { cx: 0,    cy: 0,   s: 0,   e: 90  },
          { cx: 1440, cy: 0,   s: 90,  e: 180 },
          { cx: 1440, cy: 900, s: 180, e: 270 },
          { cx: 0,    cy: 900, s: 270, e: 360 },
        ].map(({ cx, cy, s, e }, idx) => (
          <g key={`cf-${idx}`}>
            ...
            {Array.from({ length: 15 }).map((_, j) => {
              ...
              return <line key={j} x1={cx+Math.cos(a)*30} y1={cy+Math.sin(a)*30} x2={cx+Math.cos(a)*230} y2={cy+Math.sin(a)*230} stroke="#FBEAD6" strokeWidth="0.35" strokeOpacity="0.16" />;
            })}
            {[55,90,125,160,195,230].map((r, i) =>
      ...
      >
        <form
          ...
          className="relative w-full bg-[#FBEAD6]/92 backdrop-blur-xl border border-[#FBEAD6]/25 rounded-2xl px-4 py-5 sm:px-7 sm:py-6 shadow-[0_20px_60px_rgba(10,18,6,0.5)] sm:shadow-[0_32px_90px_rgba(10,18,6,0.55)]"
        >
          ...
          <div className="text-center mb-4 sm:mb-5">
            ...
            <p className="font-['Cormorant_Garamond',serif] italic text-[0.55rem] sm:text-[0.6rem] tracking-[0.28em] sm:tracking-[0.32em] uppercase text-[#C87D87]/55 mb-0.5 sm:mb-1">
              Welcome Back
            </p>
          ...
          <div className="mb-4">
            ...
            <div className="grid grid-cols-2 gap-2 sm:gap-3">
              <button
                ...
                className={`flex flex-col items-center gap-1 py-2.5 sm:py-3 min-h-[64px] sm:min-h-[72px] rounded-xl border transition-all duration-300 ${
                  selectedRole === 'user'
                    ? 'border-[#6B7556]/60 bg-[#6B7556]/10'
                    : 'border-[#C87D87]/15 bg-transparent hover:border-[#C87D87]/30 active:bg-[#6B7556]/5'
                }`}
              >
                <svg className={`w-4 h-4 sm:w-5 sm:h-5 transition-colors ${selectedRole === 'user' ? 'text-[#6B7556]' : 'text-[#C87D87]/45'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                </svg>
                <span className={`font-['Cormorant_Garamond',serif] text-[0.65rem] sm:text-xs tracking-[0.12em] sm:tracking-[0.16em] uppercase font-semibold transition-colors ${selectedRole === 'user' ? 'text-[#6B7556]' : 'text-[#7a6a5a]/70'}`}>
                  Member
                </span>
                {selectedRole === 'user' && <div className="w-3 h-px bg-[#6B7556]/60" />}
              </button>
              ...
              <button
                ...
                className={`flex flex-col items-center gap-1 py-2.5 sm:py-3 min-h-[64px] sm:min-h-[72px] rounded-xl border transition-all duration-300 ${
                  selectedRole === 'admin'
                    ...
                    : 'border-[#C87D87]/15 bg-transparent hover:border-[#C87D87]/30 active:bg-[#C87D87]/5'
                }`}
              >
                <svg className={`w-4 h-4 sm:w-5 sm:h-5 transition-colors ${selectedRole === 'admin' ? 'text-[#C87D87]' : 'text-[#C87D87]/45'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                </svg>
                <span className={`font-['Cormorant_Garamond',serif] text-[0.65rem] sm:text-xs tracking-[0.12em] sm:tracking-[0.16em] uppercase font-semibold transition-colors ${selectedRole === 'admin' ? 'text-[#C87D87]' : 'text-[#7a6a5a]/70'}`}>
                  Admin
                </span>
                {selectedRole === 'admin' && <div className="w-3 h-px bg-[#C87D87]/60" />}
              </button>
            </div>
          </div>
          ...
          {isSuspended && !error && (
            ...
            >
              ...
              <p className="font-['Cormorant_Garamond',serif] italic text-xs sm:text-sm text-[#C87D87] leading-relaxed">
            ...
            {selectedRole === 'admin' && (
              <div style={{ animation: 'adminSlide 0.35s cubic-bezier(.4,0,.2,1) forwards' }}>
                <div className="flex items-center gap-2 my-2 sm:my-3">
                  ...
                  <span className="font-['Cormorant_Garamond',serif] italic text-[0.55rem] sm:text-[0.6rem] tracking-[0.18em] sm:tracking-[0.2em] uppercase text-[#C87D87]/48 whitespace-nowrap">
                    Admin Verification
                  </span>
                ...
                <div className="group">
                  ...
                  <div className="relative">
                    <input
                      ...
                      required
                      className="w-full px-3 py-2.5 sm:px-3.5 sm:py-2 bg-[#C87D87]/5 border border-[#C87D87]/22 focus:border-[#C87D87]/50 focus:bg-[#C87D87]/8 focus:outline-none rounded-lg font-['Cormorant_Garamond',serif] italic text-[0.95rem] sm:text-sm text-[#3a3027] placeholder:text-[#7a6a5a]/35 transition-all duration-300"
                    />
                    <div className="absolute bottom-0 left-2 right-2 h-px bg-gradient-to-r from-transparent via-[#C87D87]/40 to-transparent opacity-0 group-focus-within:opacity-100 transition-opacity duration-500" />
                  </div>
                  <p className="font-['Cormorant_Garamond',serif] italic text-[0.6rem] sm:text-[0.63rem] text-[#C87D87]/40 mt-1.5 text-center">
          ...
          <div className="mt-2 text-right">
            <Link
              ...
              className="font-['Cormorant_Garamond',serif] italic text-[0.7rem] sm:text-xs text-[#C87D87]/55 hover:text-[#C87D87] transition-colors border-b border-[#C87D87]/18 pb-px"
            >
              Forgot password?
            </Link>
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className={`w-full mt-4 relative overflow-hidden font-['Cormorant_Garamond',serif] text-[0.75rem] sm:text-sm tracking-[0.24em] sm:tracking-[0.28em] uppercase text-white border-0 py-2.5 sm:py-3 min-h-[44px] sm:min-h-[48px] rounded-xl transition-all duration-300 disabled:opacity-50 group ${
