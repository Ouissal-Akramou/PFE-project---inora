...
const LaceCorner = ({ flip = false, danger = false }) => (
  <svg width={36} height={36} viewBox="0 0 36 36" fill="none" className="pointer-events-none select-none" style={{ transform: flip ? 'scaleX(-1)' : undefined }}>
    <line x1="0" y1="1" x2="22" y2="21" stroke={danger ? '#ef4444' : '#C87D87'} strokeWidth="0.8" strokeOpacity="0.50"/>
    <line x1="1" y1="0" x2="21" y2="22" stroke={danger ? '#ef4444' : '#C87D87'} strokeWidth="0.8" strokeOpacity="0.50"/>
    <rect x="2" y="2" width="4.5" height="4.5" transform="rotate(45 4.25 4.25)" fill="none" stroke={danger ? '#ef4444' : '#C87D87'} strokeWidth="0.7" strokeOpacity="0.80"/>
    <circle cx="4.25" cy="4.25" r="0.8" fill={danger ? '#ef4444' : '#C87D87'} fillOpacity="0.45"/>
    <rect x="8" y="1.5" width="3" height="3" transform="rotate(45 9.5 3)" fill="none" stroke={danger ? '#ef4444' : '#C87D87'} strokeWidth="0.5" strokeOpacity="0.30"/>
    <rect x="1.5" y="8" width="3" height="3" transform="rotate(45 3 9.5)" fill="none" stroke={danger ? '#ef4444' : '#C87D87'} strokeWidth="0.5" strokeOpacity="0.30"/>
    {[8,12,16,20].map((x,j) => <line key={x} x1={x} y1="1" x2={x} y2={j===20?43:21} stroke={danger ? '#ef4444' : '#C87D87'} strokeWidth="0.4" strokeOpacity="0.25"/>)}
    {[8,12,16,20].map((y,j) => <line key={y} x1="1" y1={y} x2={j===20?43:21} y2={y} stroke={danger ? '#ef4444' : '#C87D87'} strokeWidth="0.4" strokeOpacity="0.25"/>)}
  </svg>
)
...
const DeleteModal = ({ onConfirm, onCancel }) => (
  <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{background:'rgba(58,48,39,0.55)',backdropFilter:'blur(4px)',animation:'fadeIn .2s ease both'}}>
    <div className="bg-[#fef6ec] rounded-2xl border border-red-200/60 shadow-[0_24px_64px_rgba(58,48,39,0.25)] w-full max-w-md relative overflow-hidden" style={{animation:'fadeUp .25s ease both'}}>
      ...
      <div className="p-8">
        ...
        <p className="font-['Cormorant_Garamond',serif] text-sm text-[#5a4a3a] text-center leading-relaxed mb-8">Your account will be <span className="text-red-500 font-semibold">permanently deleted</span>. Your bookings and payment history will be preserved but your account will no longer be accessible.</p>
...
const CancelBookingModal = ({ booking, onConfirm, onCancel }) => (
  <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{background:'rgba(58,48,39,0.55)',backdropFilter:'blur(4px)',animation:'fadeIn .2s ease both'}}>
    <div className="bg-[#fef6ec] rounded-2xl border border-[#C87D87]/30 shadow-[0_24px_64px_rgba(58,48,39,0.25)] w-full max-w-md relative overflow-hidden" style={{animation:'fadeUp .25s ease both'}}>
      ...
      <div className="p-8">
        <div className="w-12 h-12 rounded-2xl bg-amber-50 border border-amber-200 flex items-center justify-center mx-auto mb-5">
          <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6 text-amber-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75}><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>
        </div>
...
export default function AccountPage() {
  ...
  useEffect(() => {
    ...
    const valid = ['personal', 'bookings', 'security', 'danger']
  ...
  const exportSinglePDF = (booking) => {
    import('jspdf').then(({ default: jsPDF }) => {
      ...
      const rows = [
        ...
        ['Guests', `${booking.participants} ${(booking.participants??1)===1?'person':'people'}`],
    ...
    <>
      ...
      <div className="min-h-screen flex" style={{animation:'fadeIn .4s ease both'}}>
            ...
            <div className="px-5 py-4 border-b border-white/8 flex-shrink-0">
              <div className="flex items-center gap-3">
                <label htmlFor="avatar-upload" className="cursor-pointer group-av flex-shrink-0 relative">
                  {avatarUrl
                    ? <img src={`${process.env.NEXT_PUBLIC_API_URL}${avatarUrl}`} alt="avatar" className="w-9 h-9 rounded-full object-cover ring-2 ring-white/20 transition-opacity group-hover:[&.av]:opacity-70"/>
                    : <div className="w-9 h-9 rounded-full bg-white/20 border border-white/30 flex items-center justify-center text-white font-bold text-sm transition-opacity group-hover:opacity-70">{displayName.charAt(0).toUpperCase()}</div>
                  }
        ...
        <main className={`${mainML} flex-1 min-h-screen dash-bg transition-all duration-300`}>
          <header className="sticky top-0 z-30 bg-[#6B7556] backdrop-blur-xl border-b border-b-[#5606a]/30 px-6 py-2 flex items-center justify-between relative" style={{boxShadow:'0 2px 24px rgba(200,125,135,0.22)'}}>
            ...
            <div className="flex items-center gap-3">
              {profile?.role && (
                <span className={`font-['Cormorant_Garamond',serif] text-[0.6rem] tracking-widest uppercase border px-2.5 py-1 rounded-full ${profile.role==='admin'?'bg-white/20 text-white border-white/30':'bg-white/15 text-white border-white/25'}`}>{profile.role}</span>
              )}
              <p className="font-['Cormorant_Garamond',serif] italic text-sm text-white/60 hidden md:block">{new Date().toLocaleDateString('fr-FR',{weekday:'long',day:'numeric',month:'long'})}</p>
            </div>
          </header>
                ...
                {bookingsLoading && (
                  <div className="flex flex-col items-center gap-4 py-20">
                    <div className="relative w-16 h-16 flex items-center justify-center">
                      <style>{`
                        @keyframes lacePulse2   { 0%,100%{opacity:.45} 50%{opacity:1} }
                        @keyframes laceRotate2  { from{transform:rotate(0deg)} to{transform:rotate(360deg)} }
                        @keyframes laceCounter2 { from{transform:rotate(0deg)} to{transform:rotate(-360deg)} }
                      `}</style>
                      <svg className="absolute inset-0 w-full h-full" viewBox="0 0 64 64" style={{animation:'laceRotate2 8s linear infinite'}}>
                        <circle cx="32" cy="32" r="29" fill="none" stroke="#C87D87" strokeWidth="0.6" strokeOpacity="0.35" strokeDasharray="3 5"/>
                        {[0,45,90,135,180,225,270,315].map((a,i) => {
                          const r = a*Math.PI/180
                          return <g key={i}>
                            <line x1={32+Math.cos(r)*14} y1={32+Math.sin(r)*14} x2={32+Math.cos(r)*29} y2={32+Math.sin(r)*29} stroke="#C87D87" strokeWidth="0.5" strokeOpacity="0.28"/>
                            <circle cx={32+Math.cos(r)*29} cy={32+Math.sin(r)*29} r="1" fill="#C87D87" fillOpacity="0.45"/>
                          </g>
                        })}
                      </svg>
                      <svg className="absolute inset-0 w-full h-full" viewBox="0 0 64 64" style={{animation:'laceCounter2 6s linear infinite'}}>
                        <circle cx="32" cy="32" r="19" fill="none" stroke="#C87D87" strokeWidth="0.7" strokeOpacity="0.38"/>
                        {[0,60,120,180,240,300].map((a,i) => {
                          const r = a*Math.PI/180
                          return <g key={i}>
                            <circle cx={32+Math.cos(r)*19} cy={32+Math.sin(r)*19} r="1.5" fill="none" stroke="#C87D87" strokeWidth="0.5" strokeOpacity="0.50"/>
                          </g>
                        })}
                      </svg>
                      <svg className="absolute inset-0 w-full h-full" viewBox="0 0 64 64" style={{animation:'lacePulse2 2s ease-in-out infinite'}}>
                        <circle cx="32" cy="32" r="9" fill="none" stroke="#C87D87" strokeWidth="0.6" strokeOpacity="0.42"/>
                        <rect x="29" y="29" width="6" height="6" transform="rotate(45 32 32)" fill="none" stroke="#C87D87" strokeWidth="0.7" strokeOpacity="0.62"/>
                        <circle cx="32" cy="32" r="1.5" fill="#C87D87" fillOpacity="0.52"/>
                      </svg>
                    </div>
                  ...
                  <Card>
                    <div className="p-12 text-center">
                      <div className="w-14 h-14 rounded-2xl bg-[#C87D87]/8 border border-[#C87D87]/15 flex items-center justify-center mx-auto mb-5">
                        <svg xmlns="http://www.w3.org/2000/svg" className="w-7 h-7 text-[#C87D87]/50" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                      </div>
                              ...
                              <div className="p-6">
                                <div className="flex items-start justify-between gap-4 mb-4">
                                  <div className="flex items-center gap-3">
                                    <div className={`w-10 h-10 rounded-xl border ${s.border} ${s.bg} flex items-center justify-center flex-shrink-0`}>
                                      <svg xmlns="http://www.w3.org/2000/svg" className={`w-5 h-5 ${s.text}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75}><path strokeLinecap="round" strokeLinejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                                    </div>
                                    <div>
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                                  {[
                                    ...
                                    { label:'Guests',  value: `${booking.participants} ${(booking.participants??1)===1?'person':'people'}` },
                                ...
                                <div className="mt-4 pt-3 border-t border-[#C87D87]/8 flex items-center justify-between">
                                  ...
                                  <div className="flex items-center gap-2">
                                    ...
                                    {(booking.status?.toLowerCase()==='pending' || (booking.status?.toLowerCase()==='confirmed' && booking.paymentStatus!=='PAID')) && (
                                      <button onClick={() => setCancelTarget(booking)} disabled={cancellingId===booking.id} className="inline-flex items-center gap-1.5 font-['Cormorant_Garamond',serif] text-[0.62rem] tracking-[0.15em] uppercase text-amber-600 border border-amber-200 bg-amber-50 px-3 py-1.5 rounded-lg hover:bg-amber-100 transition-all disabled:opacity-40">
                ...
                <Card danger>
                  <div className="p-6 flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <p className="font-['Cormorant_Garamond',serif] text-sm text-[#5a4a3a] leading-relaxed">Deleting your account is <span className="text-red-500 font-semibold">permanent and irreversible</span>. You will be logged out immediately. Your bookings and payment history will be preserved but your account will no longer be accessible.</p>
                    </div>
                  </div>
                </Card>
                <Card danger>
