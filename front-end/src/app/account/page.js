...
const LaceCorner = ({ flip = false, danger = false }) => (
  <svg width="36" height="36" viewBox="0 0 36 36" fill="none"
    ...
    <line x1="0" y1="1" x2="22" y2="22.1" stroke={danger ? '#ef4444' : '#C87D87'} strokeWidth="0.8" strokeOpacity="0.50"/>
    <line x1="1" y1="0" x2="22.1" y2="22.2" stroke={danger ? '#ef4444' : '#C87D87'} strokeWidth="0.8" strokeOpacity="0.50"/>
    ...
    {[8,12,16,20].map((x,j) => (
      <line key={x} x1={x} y1="1" x2={x} y2={j===3?4:3} stroke={danger ? '#ef4444' : '#C87D87'} strokeWidth="0.4" strokeOpacity="0.25"/>
    ))}
    {[8,12,16,20].map((y,j) => (
      <line key={y} x1="1" y1={y} x2={j===3?4:3} y2={y} stroke={danger ? '#ef4444' : '#C87D87'} strokeWidth="0.4" strokeOpacity="0.25"/>
    ))}
  </svg>
);
...
function PaymentBadge({ booking }) {
  ...
  const total      = (parseInt(booking.participants) || 1) * PRICE_PER_PERSON;
...
export default function AccountPage() {
  ...
  const handleName = async (e) => {
    ...
    try {
      const res  = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/profile/me/name`, { method: 'PATCH', credentials: 'include', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(nameForm) });
      const data = await res.json();
  ...
  const handleEmail = async (e) => {
    ...
    try {
      const res  = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/profile/me/email`, { method: 'PATCH', credentials: 'include', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(emailForm) });
      const data = await res.json();
  ...
  const handlePassword = async (e) => {
    ...
    setPassLoading(true);
    try {
      const res  = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/profile/me/password`, { method: 'PATCH', credentials: 'include', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(passForm) });
      const data = await res.json();
  ...
  const handleDelete = async (e) => {
    ...
    try {
      const res  = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/profile/me/verify-password`, { method: 'POST', credentials: 'include', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ password: deleteForm.password }) });
      const data = await res.json();
  ...
  const confirmDelete = async () => {
    ...
    try {
      const res  = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/profile/me`, { method: 'DELETE', credentials: 'include', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ password: deleteForm.password }) });
      const data = await res.json();
      if (!res.ok) { setDeleteMsg({ type: 'error', text: data.message }); return; }
